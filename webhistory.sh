...
install_addon() {
    ...
    echo "Generating WebHistory.asp..."
    cat << 'EOF1' > /jffs/addons/webhistory_report/WebHistory.asp
...
EOF1
    ...
    echo "Generating WebHistory_Report.sh..."
    cat << 'EOF2' > /opt/bin/WebHistory_Report.sh
#!/bin/sh
VER="v1.17"
...
SQL_TABLE="history"
SQL_DATABASE=
SQL_ADDONS_DATABASE="/opt/var/WebHistory_Addons.db"                             # v1.16
...
EOF2
    ...
    echo "Generating custom_webhistory..."
    cat << 'EOF3' > /jffs/scripts/custom_webhistory
#!/bin/sh
...
# Rather than a txt file, create a fake .asp endpoint in user space which httpd definitely serves
REPORT_PATH="/www/user/webhistory_output.asp"
SCRIPT_PATH="/opt/bin/WebHistory_Report.sh"

if [ -f "$SCRIPT_PATH" ]; then
    # Clear old report
    echo "<!-- WebHistory Local Report Secure Output -->" > "$REPORT_PATH"
